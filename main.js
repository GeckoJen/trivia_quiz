...
let difficulty = document.querySelector("#difficultyLevel");
let difficulties = [
...
async function getQuestions() {
  ...
  categorySelect = categories.filter(function (item) {
    return item.name === category.value;
  });

  difficultySelect = difficulties.filter(function (item) {
    return item.name === difficulty.value;
  });

  const response = await fetch(
    `https://opentdb.com/api.php?amount=${numQuestions}&category=${categorySelect[0].id}&${difficultySelect[0].id}type=multiple`
  );
